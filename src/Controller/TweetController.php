...
class TweetController extends AbstractController
{
    function index() {
        $result = array();
        $result['users'] = $this->generateUrl('ver_usuario',array(),
                                              UrlGeneratorInterface::ABSOLUTE_URL);
        $result['tweets'] = $this->generateUrl('verTweet',array(),
                                              UrlGeneratorInterface::ABSOLUTE_URL);
    ...
    function verTweet($id) {
        ...
        $result->user = $this->generateUrl('ver_usuario', [
        ...
        foreach ($tweet->getLikes() as $user) {
            $result->likes[] = $this->generateUrl('ver_usuario', [
                'id' => $user->getId(),
            ], UrlGeneratorInterface::ABSOLUTE_URL);
        }
    ...
    function verUsuario($id) {
        ...
        // Para enlazar al usuario, añadimos el enlace API para consultar su información.
        $result->tweet = $this->generateUrl('ver_tweet', [
        ...
        function postUsuario(Request $request) {
          ...
          function postTweet(Request $request) {
            $entityManager = $this->getDoctrine()->getManager();
            $tweet = $entityManager->getRepository(Tweet::class)->findOneBy(['id' => $request->request->get("id")]);
            if ($tweet) {
              return new JsonResponse([
            ...
            $tweet->setDate($request->request->get("date"));
          ...
          function putUsuario(Request $request, $id) {
            $entityManager = $this->getDoctrine()->getManager();
            $user = $entityManager->getRepository(User::class)->find($id);
            if ($user == null) {
              return new JsonResponse([
          ...
          function putTweet(Request $request, $id) {
            ...
            if ($tweet->getId() != $request->request->get("id")) {
                $tweet2 = $entityManager->getRepository(Tweet::class)->findOneBy(['id' => $request->request->get("id")]);
                if ($tweet2) {
                return new JsonResponse([
